...
def pingpong(n):
    ...
    def flag(n):
        ...
        if num_eights(n)!=0 or n%8==0:
            ...
            return flag(n-1)
    return pingpong(n-1)+flag(n-1)##做两个递归
...
def missing_digits(n):
    ...
    if x==-1:
        x=0
    return x + missing_digits(n//10)

# n=int(input())
# print(missing_digits(n))
...
def count_coins(change):
    ...
    def count(change,coin):
        ...
        if change<0:
            return 0
        if coin==25:
            return count(change-25, 25)+count(change-5, 5)+count(change-10, 10)+count(change-1, 1)
        elif coin==10:
            return count(change-5, 5)+count(change-10, 10)+count(change-1, 1)
        elif coin==5:
            return count(change-5,5)+count(change-1, 1)
        else:
            return count(change-1,1)
    return count(change-25, 25)+count(change-5, 5)+count(change-10, 10)+count(change-1, 1)
# change=int(input())
...
#我的问题在于lambda函数没有名字，没办法递归，这里用了两个形参，一个形参作为函数名，就可以递归了
